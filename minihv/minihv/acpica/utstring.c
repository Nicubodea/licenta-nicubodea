    ...
    char                    *Source)
{
    ...
    if ((strlen_s (Dest) + strlen_s (Source)) >= DestSize)
    {
        return (TRUE);
    }
    ...
    ACPI_SIZE               MaxTransferLength)
{
    ...
    ActualTransferLength = ACPI_MIN (MaxTransferLength, strlen_s (Source));

    if ((strlen_s (Dest) + ActualTransferLength) >= DestSize)
    {
        return (TRUE);
    }
